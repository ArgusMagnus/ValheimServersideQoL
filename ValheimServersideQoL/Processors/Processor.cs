        ...
        if (__methods.TryGetValue(data.m_methodHash, out var rpcMethod))
        {
            ...
            for (int i = 0; i < rpcMethod.Delegates.Count; i++)
            {
                ...
                try
                {
                    ...
                    __loopCounter++;
                    if (__loopCounter > 1)
                        Main.Instance.Logger.DevLog($"{rpcMethod.Name}: Loop Counter: {__loopCounter}");
    ...
    protected static class RPC
    {
        ...
        static void InvokeRoutedRPC(long targetPeerId, string methodName, object[]? parameters = null)
        {
            if (Main.Instance.Config.General.DiagnosticLogs.Value)
            {
                ...
                if (count % 10 is 0)
                    Main.Instance.Logger.LogInfo($"{nameof(InvokeRoutedRPC)}: {methodName}: {count} of {__invokeTotalCounter} ({(float)count/__invokeTotalCounter:P0})");
            }
        ...
        static void InvokeRoutedRPCAsSender(long senderPeerId, long targetPeerID, ZDOID targetZDO, string methodName, object[] parameters)
        {
            if (Main.Instance.Config.General.DiagnosticLogs.Value)
            {
                ...
                __invokeTotalCounter++;
                if (count % 10 is 0)
                    Main.Instance.Logger.LogInfo($"{nameof(InvokeRoutedRPCAsSender)}: {methodName}: {count} of {__invokeTotalCounter} ({(float)count / __invokeTotalCounter:P0})");
            }
