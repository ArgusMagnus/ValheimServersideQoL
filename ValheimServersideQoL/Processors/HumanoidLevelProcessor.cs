...
sealed class HumanoidLevelProcessor : Processor
{
    ...
    void SetFields<T>(ExtendedZDO zdo, int level) where T : Character
    {
        ...
        else if (fields.UpdateValue(static () => x => x.m_name, $"<line-height=150%><voffset=-2em>{(zdo.PrefabInfo.Humanoid?.Humanoid ?? zdo.PrefabInfo.Character!.Value.Character).m_name}<size=70%><br><color=yellow>{string.Concat(Enumerable.Repeat("⭐", level - 1))}</color></size></voffset></line-height>"))
            RecreateZdo = true;
    }
}