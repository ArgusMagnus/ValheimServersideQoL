...
sealed class FireplaceProcessor : Processor
{
    ...
    protected override bool ProcessCore(ExtendedZDO zdo, IReadOnlyList<Peer> peers)
    {
        ...
        if (ignoreRain)
        {
            ...
            if (fields.UpdateValue(static () => x => x.m_coverCheckOffset, offset))
                RecreateZdo = true;
            if (fields.UpdateValue(static () => x => x.m_disableCoverCheck, true))
                RecreateZdo = true;
        ...
        {
            if (fields.UpdateResetValue(static () => x => x.m_coverCheckOffset))
                RecreateZdo = true;
            if (fields.UpdateResetValue(static () => x => x.m_disableCoverCheck))
                RecreateZdo = true;
            if (_enclosure.Remove(zdo, out var enclosures))
            {
                foreach (var piece in enclosures)
