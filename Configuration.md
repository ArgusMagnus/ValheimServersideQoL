...
|Containers|AutoSort|True|True/False|True to auto sort container inventories|
|Containers|AutoPickup|True|True/False|True to automatically put dropped items into containers if they already contain said item|
...
|Doors|AutoCloseMinPlayerDistance|4||Min distance all players must have to the door before it is closed. NaN to disable this feature|
|Fireplaces|MakeToggleable|True|True/False|True to make all fireplaces (including torches, braziers, etc.) toggleable|
|Fireplaces|InfiniteFuel|True|True/False|True to make all fireplaces have infinite fuel|
...
|Map Tables|AutoUpdatePortals|True|True/False|True to update map tables with portal pins|
...
|Map Tables|AutoUpdateShips|True|True/False|True to update map tables with ship pins|
...
|Signs|TimeSigns|True|True/False|True to update sign texts which contain time emojis (any of 🕛🕧🕐🕜🕑🕝🕒🕞🕓🕟🕔🕠🕕🕡🕖🕢🕗🕣🕘🕤🕙🕥🕚🕦) with the in-game time|
|Smelters|FeedFromContainers|True|True/False|True to automatically feed smelters from nearby containers|
...
|Tames|MakeCommandable|True|True/False|True to make all tames commandable (like wolves)|
|Tames|SendTamingPogressMessages|True|True/False|True to send taming progress messages to nearby players|
|Tames|AlwaysFed|False|True/False|True to make tames always fed (not hungry)|
|Tames|TeleportFollow|True|True/False|True to teleport following tames to the players location if the player gets too far away from them|
...
|Turrets|LoadFromContainers|True|True/False|True to automatically load ballistas from containers|
|Turrets|LoadFromContainersRange|4||Required proxmity of a container to a ballista to be used as ammo source|
|Windmills|IgnoreWind|True|True/False|True to make windmills ignore wind (Cover still decreases operating efficiency though)|
